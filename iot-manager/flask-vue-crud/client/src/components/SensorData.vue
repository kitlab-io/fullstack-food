...
import LineChartZoom from './LineChartZoom.vue';
  ...
  <div class="sensor-data-container">
    ...
    <div class="chart-wrapper">
      ...
      <div v-if="!loading && !error && temperatureData.length > 0" class="chart-container">
        <timeline :items="timelineItems" :groups="[{ id: 1 }, { id: 'linechart' }]" :viewportMin="1744518653000"
          :viewportMax="1744604993000">

          <template #items-linechart="{ viewportStart, viewportEnd, group }">
            <LineChartZoom :viewportStart="viewportStart" :viewportEnd="viewportEnd" :data="temperatureData" />
          </template>
        </timeline>

        <LineChart :data="temperatureData" :title="chartTitle" :color="chartColor" :timeRange="visibleTimeRange"
          :yAxisLabel="'Temperature (°C)'" :height="300" :timeFormat="timeFormat" />
      </div>
    </div>
    ...
    <div class="timeline-section" v-if="temperatureData.length > 0">
      ...
      <timeline :groups="timelineGroups" :items="timelineItems" :viewportMin="visibleTimeRange.start"
